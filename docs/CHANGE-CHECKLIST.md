...
a tag is - it belongs in `frontend/src/domain/`, not in Python. The server has no
opinion about those any more.
**Reference:** [frontend/src/domain/slots.ts](../frontend/src/domain/slots.ts).

### Server module structure
`backend/app/api/` holds the whole API: `routes.py` for pull/push/export, `auth.py` for
signing in, and the supporting modules beside them. New endpoints go there
rather than into a new blueprint.
**Reference:** [backend/app/api/routes.py](../backend/app/api/routes.py).
...
### Models
All models are in a SINGLE file [backend/app/models.py](../backend/app/models.py). A new
...
### Client
Every write goes through `frontend/src/db/mutate.ts`, so the change and its outbox
entry land in one transaction. Views read through `live()` from
[frontend/src/lib/live.svelte.ts](../frontend/src/lib/live.svelte.ts) and never
...
There is a `backend/tests/` directory now, run with `pytest` from `backend/` (dependencies in `requirements-dev.txt`,
...
Run **both** suites before finishing: `cd backend && pytest` for the server, and
`cd frontend && npm run test` for the ports and the write path. A port from Python
is checked against output captured from the Python it replaces - see
`frontend/src/domain/__golden__*.json`; regenerate those rather than editing them
by hand.
...
- [ ] Change the class in [backend/app/models.py](../backend/app/models.py).
...
- [ ] Describe it on **both** sides of the wire:
      [backend/app/api/protocol.py](../backend/app/api/protocol.py) and
      [frontend/src/sync/types.ts](../frontend/src/sync/types.ts). These two drifting
      apart is the failure this codebase is most exposed to.
- [ ] Add the store to [frontend/src/db/schema.ts](../frontend/src/db/schema.ts).
- [ ] Generate the migration: `flask --app run.py db migrate -m "..."`, run from `backend/`
      — that is where `run.py` and `migrations/` live.
...
- [ ] If you added a model, add it to the import in `backend/app/__init__.py`.
- [ ] Update the Data model section in [ARCHITECTURE.md](ARCHITECTURE.md).

### New or changed view
- [ ] A file in [frontend/src/routes/](../frontend/src/routes/), reading through
      `live()` and writing through `db/mutate.ts`.
- [ ] Add the route to
      [frontend/src/lib/router.svelte.ts](../frontend/src/lib/router.svelte.ts) and
      the navigation in `frontend/src/App.svelte`.
...
- [ ] Add the read in [backend/app/config.py](../backend/app/config.py), the `Config` class (`os.environ.get(...)` with a sensible default).
- [ ] Add the variable with a comment and a default value to [instance/.env.example](../instance/.env.example)
...
- [ ] Update the view in [frontend/src/routes/](../frontend/src/routes/) or the component in
      [frontend/src/ui/](../frontend/src/ui/), and the style in [frontend/src/styles/](../frontend/src/styles/).
...
### New dependency
- [ ] Add a pinned entry (`name==version`) to [backend/requirements.txt](../backend/requirements.txt) — keep the version-pinning style.
- [ ] Test a clean build: `pip install -r backend/requirements.txt` in a fresh `.venv`.
